...
public class Crypto {
    ...
    private String tryEncrypt(String plainText, String key) 
        ...
        cipher.init(Cipher.ENCRYPT_MODE, this.generateKey(key));
        byte[] plainTextByteArray= plainText.getBytes();
        byte[] cryptTextByteArray= cipher.doFinal(plainTextByteArray);
    ...
    private String tryDecrypt(String cryptText, String key)
        ...
        Cipher cipher = Cipher.getInstance("AES");
        cipher.init(Cipher.DECRYPT_MODE, this.generateKey(key));
        byte[] cryptTextByteArray = Base64.getDecoder().decode(cryptText);
        byte[] plainTextByteArray = cipher.doFinal(cryptTextByteArray);
